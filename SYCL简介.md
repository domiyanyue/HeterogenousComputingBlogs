...
2. 主机端(host)程序和设备端(device)程序的弱耦合机制让其开发过程十分容易出错。开发者需要使用两种不同语言进行开发，同时使用额外的脚本去生成住机端和设备端的共享代码。
...
1. **配置运行时环境** 为与设备端交互做准备的，比如创建代表具体设备的实例，创建内存共享单元等。在大部分框架中，运行时中的变量类型（设备，内存）以头文件或者语言语法特性的方式提供。
...
5. **提交设备端代码** 有了设备端代码，主机端还需要通知设备什么时候开始执行。这个操作通过一个显示或隐式的队列完成，一组设备端代码以事件的形式提交到主队列，当设备端有足够的计算资源时，会获取并执行队列中的代码。
...
* 函数体: 函数体由仿函数(functor)形式表达。其参数列表由数据并行模型决定，本例中为`cl::sycl::item`。它的类型和使用方法不是本文的重点，读者只需知道这个参数在每个线程中有不同的值，可以用作ID来访问不同数据。本例中我们从存取器`a_acc`和b`_acc`中读取数据，计算其和，并存储在`c_acc`中。
